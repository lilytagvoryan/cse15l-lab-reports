...
You can view the code file by entering ```vim ListExamples.java```. Then begin editing the file to fix the appearing failing test. You can edit the code by inserting ```</.add><enter><n><n><n><n><j><j><x><i><2><esc><:wq><enter>```. This should not only allow you to edit the text but save it as well. The ```</.add>``` searches for anything after the slash, in this case the ```.add```. With just 4 clicks of ```<n>``` (n for the **n**ext character(s) searched) and 2 clicks of ```<j>``` (to move down a line(s)) you're brought to the line where you must make your edit. The ```<x>``` removes a character, the ```<i>``` puts the user in insert mode, and ```<2>``` is just 
