...
The ```</.add>``` searches for anything after the slash (in this case ```.add```) right before pressing ```<enter>```. Next, with just 4 clicks of ```<n>``` (for **n**ext character(s) searched) and 2 clicks of ```<j>``` (to move down a line(s)), you're brought to the line you must make your edit. The ```<x>``` removes a character, the ```<i>``` puts the user in insert mode, and the ```<2>``` replaces the existing ```1``` in order to fix the bug. The ```<esc>``` takes the user out of insert mode and into normal mode. The ```:wq:``` finally allows the user to save and quit the file right before pressing ```<enter>```. 
